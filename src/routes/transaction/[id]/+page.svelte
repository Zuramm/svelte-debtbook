...
<svelte:head>
	<title>{m.route_transaction_edit_title()} - {m.app_title()}</title>
</svelte:head>

<Header />

<main class="mx-auto max-w-2xl space-y-4 p-4">
	<h1 class="page-title">{m.route_transaction_edit_title()}</h1>
	...
	<form method="post" action="?/update">
		...
		<div class="mt-6 flex items-center justify-end gap-x-3">
			<button type="submit" class="btn-primary danger" form="formDelete"
				>{m.route_transaction_edit_button_delete()}</button
			>
			<button type="submit" class="btn-primary">{m.route_transaction_edit_button_update()}</button>
		</div>
	</form>
